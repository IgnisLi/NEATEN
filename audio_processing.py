...
def read_audiotoMFB(audio, sample_rate=c.SAMPLE_RATE,use_delta = c.USE_DELTA,use_scale = c.USE_SCALE,use_logscale = c.USE_LOGSCALE):
    #audio, sr = librosa.load(filename, sr=sample_rate, mono=True)
    #audio = audio.flatten()
...
class truncatedinputfromAudio(object):
    """Rescales the input PIL.Image to the given 'size'.
    If 'size' is a 2-element tuple or list in the order of (width, height), it will be the exactly size to scale.
    If 'size' is a number, it will indicate the size of the smaller edge.
    For example, if height > width, then image will be
    ...
    def __init__(self, input_per_file=1):
        ...
        super(truncatedinputfromAudio, self).__init__()
        self.input_per_file = input_per_file

    def __call__(self, audio):

        frames_features=read_audiotoMFB(audio)
        network_inputs = []
        num_frames = len(frames_features)
        import random
...
class to_mytensor(object):
    ...
    def __call__(self, pic):
        ...
        if isinstance(pic, np.ndarray):
            ...
            #img = torch.FloatTensor(pic.transpose((0, 2, 1)))
            while pic.size != 5* 16000:
                if pic.size > 5* 16000:
                    pic = pic[:(5*16000)]
                else:
                    pic = np.hstack((pic,pic))
            
            img=torch.FloatTensor(pic)
            #img = np.float32(pic.transpose((0, 2, 1)))
            return img
